...
class AiProcessManager {
  ...
  Future<void> ensureRunning({Duration timeout = const Duration(seconds: 30)}) async {
    ...
    final aiDataDir = localAppData != null
        ? p.join(localAppData, 'otzaria_ai')
        : null;
    final modelsDir =
        aiDataDir != null ? p.join(aiDataDir, 'models') : null;
    ...
    _proc = await Process.start(
      ...
      environment: {
        ...
        if (aiDataDir != null) 'OTZARIA_AI_DATA': aiDataDir,
        if (modelsDir != null) 'HF_HOME': modelsDir,
        if (modelsDir != null) 'TRANSFORMERS_CACHE': modelsDir,
      },
